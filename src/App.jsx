...
const App = () => {
  ...
  // --- Helper de Cálculo de Valor Efetivo ---
  const getEffectiveItemValues = (item) => {
    ...
    return {
        previsto: vPrev * share,
        realizado: vReal * share,
        share: share
    };
  };
  ...
  const filteredBudgetItems = useMemo(() => {
    ...
    items.sort((a, b) => {
        ...
        if (key === 'saldo') {
            const valA = getEffectiveItemValues(a);
            const valB = getEffectiveItemValues(b);
            aVal = valA.realizado - valA.previsto;
            bVal = valB.realizado - valB.previsto;
        }
        ...
        if (key === 'valorPrevisto') {
            const valA = getEffectiveItemValues(a);
            const valB = getEffectiveItemValues(b);
            return direction === 'asc' ? valA.previsto - valB.previsto : valB.previsto - valA.previsto;
        }

        if (key === 'valorRealizado') {
            const valA = getEffectiveItemValues(a);
            const valB = getEffectiveItemValues(b);
            return direction === 'asc' ? valA.realizado - valB.realizado : valB.realizado - valA.realizado;
        }
  ...
  const chartData = useMemo(() => {
      ...
      itemsToProcess.forEach(item => {
          ...
          data[monthIndex].previsto += values.previsto;
          data[monthIndex].realizado += values.realizado;
      });
  ...
  const budgetStats = useMemo(() => {
      ...
      filteredBudgetItems.forEach(item => {
          const values = getEffectiveItemValues(item);
          totalPrevisto += values.previsto;
          totalRealizado += values.realizado;
      });
