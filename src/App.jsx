...
const App = () => {
  ...
  const addAllocation = () => {
      ...
      setFormData({
          ...formData,
          allocations: [...formData.allocations, { squad: SQUADS[0], percent: 0 }]
      });
  };
        ...
        <main className="max-w-7xl mx-auto p-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            ...
            <div className="bg-[#244C5A] p-8 rounded-3xl shadow-xl text-white relative overflow-hidden flex flex-col justify-between">
              ...
              <div><h3 className="text-2xl font-bold">{loading ? "..." : formatCurrency(stats.sumCLT)}</h3><p className="text-white/50 font-bold uppercase text-[9px] tracking-widest">Folha CLT (Provisionada)</p></div>
            </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 relative overflow-hidden flex flex-col justify-between">
              ...
              <div><h3 className="text-2xl font-bold text-[#244C5A]">{loading ? "..." : formatCurrency(stats.sumEstagio)}</h3><p className="text-slate-400 font-bold uppercase text-[9px] tracking-widest">Bolsas (Provisionadas)</p></div>
            </div>
          </div>
          ...
          <section className="bg-white p-10 rounded-[40px] shadow-2xl border border-slate-50 animate-in slide-in-from-bottom duration-500">
             <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-10 gap-6">
                <div>
                   <div className="flex items-center gap-2 text-[#0097A9] mb-1 uppercase text-[10px] font-black tracking-widest"><PieChart size={14}/> Análise Orçamentária</div>
                   <h2 className="text-2xl font-black text-[#244C5A]">Previsão Mensal por Squad (Rateio)</h2>
                </div>
                   ...
                   {stats.squadStats.length > 0 ? stats.squadStats.map((s, idx) => (
                      <div key={s.name} className="group">
                         <div className="flex justify-between items-end mb-2">
                            <div><span className="text-[9px] font-black text-slate-300 uppercase tracking-widest mr-2">0{idx + 1}</span><span className="text-sm font-bold text-[#244C5A] group-hover:text-[#0097A9] transition-colors">{s.name}</span></div>
                            <div className="text-right"><p className="text-xs font-black text-[#244C5A]">{formatCurrency(s.total)}</p><p className="text-[9px] font-bold text-[#FFC72C] uppercase tracking-tighter">{s.percent.toFixed(1)}% do orçamento</p></div>
                         </div>
                         <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-[#0097A9] rounded-full transition-all duration-1000 shadow-[0_0_8px_rgba(0,151,169,0.3)]" style={{ width: `${s.percent}%` }}/></div>
                      </div>
                   )) : <div className="py-12 text-center text-slate-400 italic bg-slate-50 rounded-2xl border border-dashed border-slate-200">Sem dados.</div>}
                </div>
             </div>
          </section>
        </main>
      ...
      <div className="max-w-7xl mx-auto -mt-12 px-8">
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-100">
          <div className="p-6 border-b flex justify-between items-center bg-slate-50/50">
            <div className="relative w-1/3"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="text" placeholder="Filtrar por nome ou squad..." className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-2xl outline-none shadow-sm" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
                        ...
                        <div className="space-y-1.5">
                            {allocations.map((a, i) => (
                                <div key={i} className="flex items-center gap-2">
                                    <span className="text-[#0097A9] font-bold text-xs">{a.squad}</span>
                                    <span className="text-slate-300 text-[9px] font-black">{a.percent}%</span>
                                </div>
                            ))}
                            <p className="text-slate-500 text-[10px] italic border-t border-slate-50 pt-1 mt-1 line-clamp-1">{emp.cargo}</p>
                        </div>
                      </td>
               ...
               <div className="w-1/2 overflow-y-auto bg-white p-10">
                 {isAdding1on1 ? (
                   <form onSubmit={handleSubmit1on1} className="space-y-6 animate-in slide-in-from-right duration-300">
                     ...
                     <div><label className="text-[10px] font-black uppercase text-slate-400 block mb-2 tracking-widest">Próxima Pauta</label><textarea value={form1on1.proximaPauta} onChange={e => setForm1on1({...form1on1, proximaPauta: e.target.value})} rows="3" className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:border-[#0097A9] text-sm resize-none"></textarea></div>
            ...
            <form onSubmit={handleSubmitEmployee} className="p-8 grid grid-cols-2 gap-6 max-h-[70vh] overflow-y-auto">
              ...
              <div className="col-span-2 bg-slate-50 p-6 rounded-3xl border border-slate-100">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button type="button" onClick={addAllocation} disabled={formData.allocations.length >= 5} className="flex items-center gap-1 text-[10px] font-black uppercase text-[#0097A9] hover:bg-[#0097A9]/10 px-3 py-1.5 rounded-xl transition-all disabled:opacity-30"><PlusCircle size={14}/> Add Squad</button>
                </div>
